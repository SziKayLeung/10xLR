...
dir <- "C:/Users/sl693/OneDrive - University of Exeter/ExeterPostDoc/1_Projects/10XSingleCell/EX165_PFC/postCellClassification/"

# SQANTI classification file
class.files.names <- c(paste0(dir, "EX165_PFC_PFC_collapsed_RulesFilter_result_classification.txt"))
...
class.files.keepMonoExonic <- class.files
...
median(class.files.keepMonoExonic$length)
mean(class.files.keepMonoExonic$length)
# plot length distribution
pdf(paste0(dir, "lengthDistributionPrePostMonoExonic.pdf"), width = 12, height = 10)
plot_grid(
  ggplot(class.files.keepMonoExonic, aes(x = length)) + geom_histogram() + theme_classic() + labs(title = "Post sqanti") + ylim(0,1200000),
  ggplot(class.files, aes(x = length)) + geom_histogram() + theme_classic() + labs(title = "Post sqanti, post filter of monoexonic isoforms") + ylim(0,1200000)
) 
dev.off()
...
barcode <- fread(paste0(dir, "EX165_PFCputative_bc_filtered_annotated_isoform_simplifed.csv"))
...
class.files.keepMonoExonic <- merge(class.files.keepMonoExonic, abundance, by.x = "isoform", by.y = 0)

# write output abundance
write.csv(class.files %>% select(isoform, contains("_FL")), paste0(dir,"cellSpecificAbundance.csv"), quote = F, row.names = F)

# filter by isoforms with less than total 2 reads across all cell populations
class.files.keepMonoExonic <- class.files.keepMonoExonic %>%  mutate(FL = rowSums(select(., contains("_FL")), na.rm = TRUE))
...
message("Number of reads from sqanti filtering:", sum(class.files.keepMonoExonic$FL))
...
plot_cellType_by_gene <- function(gene){
  ...
  p <- df %>% select(contains("FL"),structural_category, isoform) %>% 
    ...
    mutate(cellType = word(cellType, c(1), sep = "_")) %>%
