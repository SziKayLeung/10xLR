...
outputdir = "/lustre/projects/Research_Project-MRC190311/longReadSeq/ONTRNA/Gina/analysis/6_blaze/5_postClassification/"
...
barcodes = fread(paste0(outputdir,"NP16_239_PFCputative_bc.csv"), data.table = F)
# readstat = output from Iso-Seq collapse
readstat <- fread(paste0(dir, "1_blaze/4_cupcake/NP16_239_PFC_collapsed_split2.read_stat.txt")
...
# save output
write.csv(longReadBarcodesFiltered, paste0(outputdir, "NP16_239_PFCputative_bc_filtered.csv"), row.names = F, quote = F)
...
write.csv(mergedFinalIsoform, paste0(outputdir, "NP16_239_PFCputative_bc_filtered_annotated_isoform.csv"), quote = F, row.names = F)
...
write.csv(mergedFinalIsoformFilteredSimplifed, paste0(outputdir, "NP16_239_PFCputative_bc_filtered_annotated_isoform_simplifed.csv"), quote = F, row.names = F)
